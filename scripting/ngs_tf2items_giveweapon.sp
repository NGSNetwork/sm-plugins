...
Handle hItemInfoTrie = null;
#if defined _visweps_included
bool bVisWeps = false;
#endif
#if defined _tf2idb_included
bool bTF2ItemsDatabase = false;
Handle hCvarTF2IDB = null;
#endif
//int rnd_isenabled;
int iCvarNotify;
...
//Handle hMaxHealth;
Handle hChargeTimer[MAXPLAYERS + 1] = { null, ... };
...
public void OnPluginStart()
{
	...
	ConVar cv_gimme = CreateConVar("tf2items_allow_gimme", "1", "0 to disallow the use of sm_gimme, 1 to allow", 0);
...
#endif
#if defined _tf2idb_included
	hCvarTF2IDB = CreateConVar("tf2items_use_tf2idb", "1", "Enable/disable tf2idb plugin usage", 0);
	bTF2ItemsDatabase = LibraryExists("tf2idb");
#endif
	CreateItemInfoTrie();
	TF2_SdkStartup();
...
#endif
#if defined _tf2idb_included
	if(strcmp(strName, "tf2idb", false) == 0)
	{
		bTF2ItemsDatabase = true;
		CreateItemInfoTrie();
	}
#endif
}
...
stock void InstantDisguise(int client, int victim)
{
	...
	if (team != TFTeam_Red && team != TFTeam_Blue) team = ((view_as<TFTeam>(GetClientTeam(client)) == TFTeam_Red) ? (TFTeam_Blue) : (TFTeam_Red));
	TF2_DisguisePlayer(client, team, TF2_GetPlayerClass(victim), victim);
	TF2_SetPlayerClass(client, class, _, false);
}

stock void CreateDisguiseWeapon(int client, int victim)
{
	char formatBuffer[32], weaponClassname[64];
	...
	if (attribCount > 1) {
		...
		for (int i = 0; i < attribCount; i+=2) {
			...
			switch (attrIdx)
			{
				...
				{
					// attrVal = StringToInt(weaponAttribsArray[i+1]);
...
#if defined _tf2idb_included
void MakeTF2DBTrie(Handle trie)
{
	Handle hItemsArray = TF2IDB_FindItemCustom("SELECT id FROM tf2idb_item WHERE slot IN ('primary','secondary','melee','pda','pda2','building') AND has_string_attribute=0");
	if (GetArraySize(hItemsArray) <= 0)
	{
		LogMessage("[TF2Items GiveWeapon] TF2ItemsDatabase found no items. Loading hardcoded items...");
		CloseHandle(hItemsArray);
		return;
	}
	else
		LogMessage("[TF2Items GiveWeapon] TF2ItemsDatabase loaded %d items.", GetArraySize(hItemsArray));

	SortADTArray(hItemsArray, Sort_Ascending, Sort_Integer);

	int iItemDefID, iItemSlot, iItemQuality, iItemLevel;
	char strItemClass[64], strTrieName[16];

	int a, iAttributeNum, ammo;
	char strAttributes[(4 + 3 + 8) * 16 + 1];
	char appendBuffer[64];
	char strFloat[10];
	Handle ammotrie = MakeAmmoTrie();
	for (int i = 0; i < GetArraySize(hItemsArray); i++)
	{
		iItemDefID = GetArrayCell(hItemsArray, i);
		TF2IDB_GetItemClass(iItemDefID, strItemClass, sizeof(strItemClass));
		iItemSlot = view_as<int>(TF2IDB_GetItemSlot(iItemDefID));
		if (iItemDefID == 735 || iItemDefID == 736 || iItemDefID == 810 || iItemDefID == 831 || iItemDefID == 933) iItemSlot = 1;
		iItemQuality = view_as<int>(TF2IDB_GetItemQuality(iItemDefID));
		int minLev, maxLev;
		TF2IDB_GetItemLevels(iItemDefID, minLev, maxLev);
		iItemLevel = maxLev;
...
#if defined _tf2idb_included
	if (bTF2ItemsDatabase && GetConVarBool(hCvarTF2IDB))
	{
		MakeTF2DBTrie(hItemInfoTrie);
	}
#endif
//bat
...
public Action TF2_CalcIsAttackCritical(int client, int weapon, char[] weaponname, bool &result)
{
			...
			|| StrEqual(weaponname, "tf_weapon_knife")))
	{
		...
		CreateTimer(0.4, Timer_ResetMeleeCrit, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
...
{
	if (bSDKStarted == false || hSDKEquipWearable == null)
	{
		TF2_SdkStartup();
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	if ((buttons & IN_ATTACK2) && !(iLastButtons[client] & IN_ATTACK2))
	{
		...
		int targe = -1;
		if ((buttons & IN_RELOAD) && GetEntityMoveType(client) != MOVETYPE_NONE && ((targe = FindPlayerTarge(client)) != -1) && hChargeTimer[client] == null && GetEntPropFloat(client, Prop_Send, "m_flMaxspeed") > 1.01 && (class != TFClass_DemoMan || (wep == GetPlayerWeaponSlot(client, TFWeaponSlot_Melee) && (idxslot2 == 265 || idxslot2 == 20 || idxslot2 == 207 || idxslot2 == 130))))
		{
			float chargetime;
...
stock bool SetShieldBashUsed(int shield, bool used = true)
{
...
stock void DoTeleportParticles(int client)
{
	...
	char name[32];
	name = (view_as<TFTeam>(GetClientTeam(client)) == TFTeam_Blue ? "player_sparkles_blue" : "player_sparkles_red");
	if (TE_SetupTFParticle(name, pos, _, _, client, 3, 0, false))
		TE_SendToAll(0.0);
	name = (view_as<TFTeam>(GetClientTeam(client)) == TFTeam_Blue ? "teleported_blue" : "teleported_red");
