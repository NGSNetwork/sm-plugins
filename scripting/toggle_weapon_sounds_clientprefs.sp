...
public OnPluginStart()
{
	...
	g_hClientCookie = RegClientCookie("sm_weaponsound", "Toggle hearing weapon sounds", CookieAccess_Private);
...
PrepareMenu(client)
{
	Menu menu = new Menu(YesNoMenu, MENU_ACTIONS_DEFAULT|MenuAction_DrawItem|MenuAction_DisplayItem|MenuAction_Display);
	menu.SetTitle("Weapon Sounds -> ");
	menu.AddItem("0", "On");
	menu.AddItem("1", "Off");
...
public int YesNoMenu(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
...
public Action Hook_NormalSound(clients[64], &numClients, char sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	// PrintToServer("[Weapon] playing sample %s.", sample);
	if (!g_bHooked || StrContains(sample, "weapon", false) == -1)
