...
//	request.Send();
	...
	char url[1024];
	Format(url, sizeof(url), "%s?key=%s&format=vdf&names=1", BACKPACK_TF_URL, key);
...
public OnBackpackTFComplete(bool successful, const char[] error, System2HTTPRequest request, System2HTTPResponse response, HTTPRequestMethod method) {
...
public Action:Command_Backpack(client, args) {
	...
	GetClientAuthId(target, AuthId_SteamID64, steamID, sizeof(steamID)); // we could use the regular Steam ID, but we already have SteamTools, so we can just bypass backpack.tf's redirect directly
	decl String:url[256];
